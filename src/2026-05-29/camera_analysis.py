...
class MinimalPublisher(Node):
    ...
    def find_redline(self):
        img2=cv.resize(self.img, (self.WIDTH//4, self.HEIGHT//4), interpolation=cv.INTER_AREA)
    ...
    def find_band(self, gray, canny, way):
        v = int(self.vh)+50 if way == 1 else int(self.HEIGHT -50)
        r_name = "B_find+" if way == 1 else "B_find-"
        k = 0
        while self.vh < v < self.HEIGHT:
            k += 1
    ...
    def loop_item(self):
        ...
        self.output = []
        self.result["u1"],self.result["v1"],self.result["x1"], self.result["y1"] = self.find_band(gray, self.img_canny, -1)
    ...
    def publish_img_cam(self):
        ...
        for o in self.output:
            if "u" in o and "v" in o:
                cv.circle(self.img_cam, (o["u"], o["v"]), 5, (255, 0, 0), -1)

        """
        if u1 != -1:
            cv.circle(img_draw, (u1, v1), 5, (255, 0, 0), -1)
        if u2 != -1:
            cv.circle(img_draw, (u2,v2), 5, (0, 255, 0), -1)
        """
        #cv.line(img_draw,(0,self.HEIGHT // 8),(self.WIDTH,self.HEIGHT // 8), (155,0,0),1)
        #cv.line(img_draw,(0,self.HEIGHT * 7 // 8),(self.WIDTH,self.HEIGHT * 7 // 8), (0,155,0),1)
        #cv.line(img_draw,(int(self.WIDTH//2-self.perspective_width[0]//2),0),(int(self.WIDTH//2-self.perspective_width[self.HEIGHT-1]//2),self.HEIGHT), (0,0,155),1)
        #cv.line(img_draw,(int(self.WIDTH//2+self.perspective_width[0]//2),0),(int(self.WIDTH//2+self.perspective_width[self.HEIGHT-1]//2),self.HEIGHT), (0,0,155),1)
        self.publisher_img_cam.publish(self.br.cv2_to_imgmsg(self.img_cam))
    ...
    def publish_text_report(self):
        ...
        t = (end - self.publish_start) / self.tick_frequency
