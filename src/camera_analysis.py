...
class CameraAnalysis(Node):
    def __init__(self):
        ...
        super().__init__('camera_analysis')
        self.subscription = self.create_subscription( Image, "video_frame", self.image_callback, 10)
    ...
    def define_lines(self):
        ...
        while True:
            ...
            self.target_line.append(v)
        for v in range(10):
            v = self.HEIGHT - k * 10 - 1
            self.error_line.append(v)
    ...
    def analyse_line(self,gray, canny, v):
        ...
        if bord.shape[0]<3: 
            return -1 # we want to have at least 4 edges.
    ...
    #@chronometre
    def find_target_band(self, gray, canny):
        for v in self.target_line: 
            u = self.analyse_line(gray, canny, v)
            if u != -1:
                print(u,self.u0,v,self.vh)
    ...
    #@chronometre
    def find_error_band(self, gray, canny):
        for v in self.error_line: 
            u = self.analyse_line(gray, canny, v)
            if u != -1:
                return u/self.WIDTH - 0.5
        return -9999
    ...
    def image_callback(self, msg):
        ...
        self.result["u1"],self.result["v1"],self.result["x1"], self.result["y1"] = self.find_target_band(gray, self.img_canny)
        self.result["error"] = self.find_error_band(gray, self.img_canny)
    ...
    def publish_img_cam(self,frame):
        cv.circle(frame, (self.result["u1"], self.result["v1"]), 5, (255, 0, 0), -1)
